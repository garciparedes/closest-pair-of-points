    ...
    ******************************************************************************
    ...
    def getClosestDivideConquer(self):
        ...
        for i in range(0, self.dimension):
            sortDimens.append(sorted(self.pointList, key=lambda point: point.vector[i]))

        return self.divide(sortDimens, 0)
    ...
    def divide(self, initPointList, i):
        ...
        if (len(initPointList[i]) > 3):

            midPoint = initPointList[i][len(initPointList[i])/2]

            particionIzquierda = list()
            particionDerecha = list()
            for k in initPointList:
                listaIzquierda = list()
                listaDerecha = list()
                for punto in k:
                    if(punto.vector[i] < midPoint.vector[i]):
                        listaIzquierda.append(punto)
                    else:
                        listaDerecha.append(punto)

                particionIzquierda.append(listaIzquierda)
                particionDerecha.append(listaDerecha)





            leftClosestPair = self.divide(particionIzquierda,i)
            rightClosestPair = self.divide(particionDerecha,i)
            ...
            closestPair = self.getClosestBrutePlus(initPointList[i])
        return closestPair
    ...
    def conquer(self, initPointList, closestPair, i):
        ...
        finalPointList = list()
        leftPointList = initPointList[i][:len(initPointList)/2]
        rightPointList = initPointList[i][len(initPointList)/2:]
        ...
        if (len(finalPointList) >= 2):
            if (len(finalPointList) >= len(initPointList)):
                return self.getClosestBrutePlus(finalPointList)
            else:
                return self.getClosestBrutePlus(finalPointList)

                #return  self.divide(particion, i)
