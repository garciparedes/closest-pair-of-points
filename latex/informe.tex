...
\section{Solución implementada (Divide y vencerás)}
	...
	\subsection{Pseudocódigo}
		...
		\begin{algorithm}
			...
			\Procedure{closestPair}{$PLIST, i, dim$} 
				\If{$len(PLIST) < 3$}
				...
				\Else
					...
					\State $LPLIST \gets PLIST[:len(PLIST)/2]$
					
					\State $RPLIST \gets PLIST[len(PLIST)/2:]$
					
					\State $leftCPAIR \gets closestPair(LPLIST, i, dim)$
					
					\State $rightCPAIR \gets closestPair(RPLIST, i, dim)$
					
					\State $CPAIR \gets min(leftCPAIR, righCPAIR)$
					
					\State $d \gets CPAIR.distance()$
					
					\State $MPLIST \gets closePLIST(d, LPLIST, RPLIST)$
					
					\State $MPAIR \gets closestPair(MPLIST, i , dim)$
					
					\State \textbf{return} $min(CPAIR, MPAIR)$
		...
		\begin{algorithm}
			...
			\Procedure{closePLIST}{$PLIST, i, dim$} 
				\If{$len(PLIST) < 3$}
